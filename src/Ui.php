...
class Ui
{
    ...
    public function index(array $content = null)
    {
        ...
        $tmpl = new Api\Etemplate('ai-assistant.index');
        ...
        $tmpl->exec('ai-assistant.'.self::class.'.index', $content);
    }
    ...
    public function edit(array $content = null)
    {
        ...
        if (!$is_admin && !$GLOBALS['egw']->acl->check('run', 1, 'ai-assistant')) {
            Api\Framework::message('You do not have permission to access settings', 'error');
            $GLOBALS['egw']->framework->render('<p>Access denied</p>', 'AI Assistant Settings', true);
            return;
        }

        $tmpl = new Api\Etemplate('ai-assistant.edit');
        $bo = new Bo();
        $so = new So();
        ...
        if ($_POST['save']) {
            try {
                ...
                if ($is_admin) {
                    ...
                    $so->save_config('ai_api_url', $_POST['ai_api_url']);
                    $so->save_config('ai_api_key', $_POST['ai_api_key']);
                    $so->save_config('ai_model', $_POST['ai_model']);
                    $so->save_config('max_history_length', $_POST['max_history_length']);
                    $so->save_config('temperature', $_POST['temperature']);
                    $so->save_config('max_tokens', $_POST['max_tokens']);
                }
                
                // Save user preferences
                $account_id = $GLOBALS['egw_info']['user']['account_id'];
                $so->save_user_config($account_id, 'user_chat_enabled', $_POST['user_chat_enabled']);
                $so->save_user_config($account_id, 'auto_save_conversations', $_POST['auto_save_conversations']);
                $so->save_user_config($account_id, 'show_tool_details', $_POST['show_tool_details']);
        ...
        if ($_POST['test_connection']) {
            try {
                // Test API connection
                $config = [
                    'api_url' => $_POST['ai_api_url'] ?: $so->get_config('ai_api_url'),
                    'api_key' => $_POST['ai_api_key'] ?: $so->get_config('ai_api_key'),
                    'model' => $_POST['ai_model'] ?: $so->get_config('ai_model')
                ];
        ...
        $tmpl->exec('ai-assistant.'.self::class.'.edit', $content);
    }
    ...
    public function list(array $content = null)
    {
        ...
        $tmpl = new Api\Etemplate('ai-assistant.list');
        $bo = new Bo();
        
        $content = [
            'nm' => [
                'get_rows' => 'ai-assistant.'.Bo::class.'.get_rows',
                'no_filter' => true,
                'no_filter2' => true,
                'no_cat' => true,
                'order' => 'created',
                ...
                'header_left' => 'ai-assistant.list.header_left',
                'header_right' => 'ai-assistant.list.header_right',
            ]
        ];

        $tmpl->exec('ai-assistant.'.self::class.'.list', $content);
    }
    ...
    public function dialog(array $content = null)
    {
        ...
        $tmpl = new Api\Etemplate('ai-assistant.dialog');
        ...
        $tmpl->exec('ai-assistant.'.self::class.'.dialog', $content);
    }
    ...
    public function action($action, $selected, $select_all, &$success, &$failed, &$action_msg, $checkboxes = 'nm', &$msg = '')
    {
        ...
        if ($select_all)
        {
            $query = Api\Cache::getSession('ai-assistant', 'index');
