...
egw.extend('json', egw.MODULE_WND_LOCAL, function (_app, _wnd) {
     ...
     * @param {egw} _egw
     */
    const json_request = function (_menuaction, _parameters, _callback, _context, _async, _sender, _egw) {
        // Copy the parameters
    ...
    json_request.prototype.openWebSocket = function (url, tokens, account_id, error, reconnect) {
        ...
        json_request.prototype.sendRequest = function (async, method, error) {
            ...
            if (this.async) {
                ...
                promise = _wnd.fetch(url, Object.assign(Object.assign({}, init), signal))
                    .then((response) => {
    ...
    json_request.prototype.handleError = function (response, _err) {
        ...
        if (_err !== 'abort') {
            ...
            this.egw.message.call(this.egw, this.egw.lang('A request to the EGroupware server returned with an error') +
    ...
    json_request.prototype.handleResponse = function (data) {
        if (data && typeof data.response != 'undefined') {
            ...
            for (var i = 0; i < data.response.length; i++) {
                ...
                var res = data.response[i];
                if (typeof res.type == 'string' && res.type != 'data')
                    only_data = false;
                    ...
                    if (typeof handler_level[res.type] !== 'undefined') {
                        ...
                        for (let j = handlerCount - 1; j >= 0; j--) {
                            try {
                                ...
                                const promise = plugin.callback.call(plugin.context ? plugin.context : this.context, res.type, res, this);
    ...
    var json = {
         ...
         */
        json: function (_menuaction, _parameters, _callback, _context, _async, _sender) {
            return new json_request(_menuaction, _parameters, _callback, _context, _async, _sender, this);
        },
        /**
                ...
                for (var i = 0; i < parts.length; ++i) {
                    ...
                    // check if we need a not yet included app.js object --> include it now and return a Promise
                        ...
                        return _wnd.egw_import(this.webserverUrl + '/' + parts[1] + '/js/app.min.js?' + ((new Date).valueOf() / 86400 | 0).toString())
                            .then(() => this.applyFunc(_func, args, _context || _wnd), (err) => { console.error("Failure loading /" + parts[1] + '/js/app.min.js' + " (" + err + ")\nAborting."); });
                    }
    ...
    json.registerJSONPlugin(function (type, res, req) {
        ...
        if ((typeof res.data.message != 'undefined') &&
            ...
            req.alertHandler(res.data.message, res.data.details);
            return true;
        }
    ...
    json.registerJSONPlugin(function (type, res, req) {
        if (typeof res.data == 'string') {
            try {
                ...
                req.egw.debug('error', 'Error while executing script: ', res.data, e);
            }
            return true;
        }
        throw 'Invalid parameters';
    ...
    json.registerJSONPlugin(function (type, res, req) {
        if (typeof res.data.select == 'string' &&
            ...
            try {
                ...
                req.egw.debug('error', 'Function', res.data.func, 'Parameters', res.data.parms);
            }
            return true;
        }
        throw 'Invalid parameters';
