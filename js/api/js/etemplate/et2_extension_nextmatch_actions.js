...
export function nm_action(_action, _senders, _target, _ids) {
    ...
    if (_action.checkbox && (!_action.data || typeof _action.data.nm_action == 'undefined'))
        return;
    if (typeof _action.data == 'undefined' || !_action.data)
        _action.data = {};
    if (typeof _action.data.nm_action == 'undefined' && _action.type == 'popup')
    ...
    switch (_action.data.nm_action) {
        ...
        case 'long_task':
            ...
            if (doLongTask(idsArr, _ids.all, _action, mgr.data.nextmatch))
                break;
        // Fall through
        case 'egw_open':
...
export function fetchAll(ids, nextmatch, callback) {
    if (!nextmatch || !nextmatch.controller)
        return false;
    ...
    if (nextmatch.controller._grid && nextmatch.controller._grid.getTotalCount() > ids.length) {
        ...
        function () { count = total; cancel = true; }, egw.lang('Loading'), egw.lang('please wait...'), {}, [
...
export function nm_open_popup(_action, _selected) {
    ...
    var popup = ((nm === null || nm === void 0 ? void 0 : nm.getInstanceManager().DOMContainer) || document.body).querySelector("et2-dialog[id*='" + _action.id + "_popup']") || document.body.querySelector("#" + (uid || "") + "_" + _action.id + "_popup") || document.body.querySelector("[id*='" + _action.id + "_popup']");
    if (popup && popup instanceof Et2Dialog) {
        popup.show();
    }
    else if (popup) {
        let dialog = new Et2Dialog(nm === null || nm === void 0 ? void 0 : nm.egw());
        ...
        popup.querySelectorAll('et2-button').forEach((button, index) => {
            ...
            button.onclick = (e) => {
                ...
                dialog.hide();
                return button_click === null || button_click === void 0 ? void 0 : button_click.apply(button, e.currentTarget);
            };
