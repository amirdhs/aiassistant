...
exports.et2_compileLegacyJS = void 0;
/*egw:uses
    et2_interfaces;
    et2_core_common;
*/
...
function et2_compileLegacyJS(_code, _widget, _context) {
    ...
    // interface
    if (!context && _widget.implements(et2_core_interfaces_1.et2_IDOMNode)) {
    ...
    try {
        ...
        if (typeof _code == 'string' && _code.indexOf('app') == 0 && _code.split('.').length >= 3 && _code.indexOf('(') == -1) {
            var parts_1 = _code.split('.');
            var app = _widget.getInstanceManager().app_obj;
            // check if we need to load the object
            if (parts_1.length === 3 && typeof app[parts_1[1]] === 'undefined') {
                return function (ev, widget) {
                    return egw_global_1.egw.applyFunc(_code, [ev, widget]);
                };
            }
            // Code is app.appname.function, add the arguments so it can be executed
            _code += '(ev,widget)';
    ...
    return function (ev, widget) {
        ...
        egw_global_1.egw.debug('log', 'Executing legacy JS code: ', _code);
        if (arguments && arguments.length > 2) {
            egw_global_1.egw.debug('warn', 'Legacy JS code only supports 2 arguments (event and widget)', _code, arguments);
        }
        // Return the result of the called function
        return func.call(context, ev, widget || _widget);
    };
}
exports.et2_compileLegacyJS = et2_compileLegacyJS;
/**
...
function js_pseudo_funcs(_val, widget) {
    ...
    if (_val.indexOf('form::name(') != -1) {
        ...
        document.et2_form_name = et2_core_common_1.et2_form_name;
    ...
    if (_val.indexOf("xajax_doXMLHTTP('etemplate.") != -1) {
        _val = _val.replace(/^xajax_doXMLHTTP\('etemplate\.([a-z]+_widget\.[a-zA-Z0-9_]+)\'/, "xajax_doXMLHTTP('" + egw_global_1.egw.getAppName() + ".$1.etemplate'");
    }
