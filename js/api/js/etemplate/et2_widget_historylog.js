...
var et2_historylog = /** @class */ (function (_super) {
    ...
    et2_historylog.prototype.doLoadingFinished = function () {
        ...
        if (tab) {
            ...
            var handler = function (e) {
                if (typeof _this.dataview == "undefined") {
                    _this.finishInit();
                }
                // TODO: Find a better way to get this to wait
                window.setTimeout(_this._resize, 10);
            };
    ...
    et2_historylog.prototype.createWidgets = function () {
        ...
        for (var i_1 = 0; i_1 < et2_historylog.columns.length; i_1++) {
            var column = et2_historylog.columns[i_1];
            if (column.widget_type) {
                // Status ID is allowed to be remapped to something else.  Only affects the widget ID though
                var attrs = { 'readonly': true, 'id': (i_1 == et2_historylog.FIELD ? this.options.status_id : column.id) };
                column.widget = Et2Widget_1.loadWebComponent(column.widget_type, attrs, this);
        ...
        this.fields = {};
        var labels = et2_historylog.columns[et2_historylog.FIELD].widget.select_options;
        // Custom fields - Need to create one that's all read-only for proper display
        var cf_widget = et2_core_widget_1.et2_createWidget('customfields', { 'readonly': true }, this);
        ...
        var _loop_1 = function (key_1) {
            // Add label
            var option = labels.find(function (option) { return option.value == et2_extension_customfields_1.et2_customfields_list.PREFIX + key_1; });
            if (option && !option.label) {
                option.label = cf_widget.options.customfields[key_1].label;
            }
            else {
                labels.push({
                    value: et2_extension_customfields_1.et2_customfields_list.PREFIX + key_1,
                    label: cf_widget.options.customfields[key_1].label
                });
            }
            // If it doesn't support detached nodes, just treat it as text
            if (cf_widget.widgets[key_1].getDetachedNodes) {
                nodes = cf_widget.widgets[key_1].getDetachedNodes();
                ...
                this_1.fields[et2_extension_customfields_1.et2_customfields_list.PREFIX + key_1] = {
        ...
        for (var key in this.options.value['status-widgets']) {
            ...
            if (widget.instanceOf(et2_widget_selectbox_1.et2_selectbox))
        ...
        var diff = Et2Widget_1.loadWebComponent('et2-diff', {}, this);
        this.diff = {
            widget: diff,
            nodes: diff
        };
    ...
    et2_historylog.prototype._create_widget = function (key, field, attrs, options) {
        ...
        if (typeof field === 'object') {
            ...
            for (var j in field) {
                ...
                if (et2_core_widget_1.et2_registry[field[j]] && ['template'].indexOf(field[j]) < 0 || customElements.get(field[j]) ||
            ...
            if (need_box) {
                ...
                widget = Et2Widget_1.loadWebComponent('et2-vbox', attrs, this);
                for (var i in field) {
                    var type = field[i];
                    var child_attrs = jQuery.extend({ readonly: true }, attrs);
                    if (typeof type === 'object') {
                        child_attrs['select_options'] = FindSelectOptions_1.cleanSelectOptions(field[i]);
                        type = 'et2-select';
                    }
                    else {
                ...
                attrs['select_options'] = FindSelectOptions_1.cleanSelectOptions(field);
            }
        }
        ...
        if (widget === null) {
            ...
            var tries = [field, "et2-" + field + "_ro", "et2-" + field];
            for (var i = 0; i < tries.length && !widget; i++) {
                if (typeof window.customElements.get(tries[i]) !== "undefined") {
                    widget = Et2Widget_1.loadWebComponent(tries[i], attrs, this);
                }
            }
            if (!widget) {
                widget = et2_core_widget_1.et2_createWidget(typeof field === 'string' ? field : 'select', attrs, this);
                if (typeof field === "string") {
                    console.log("History specified legacy widget '" + field + "' for " + key + ".  Please change in PHP source.");
                }
            }
        }
        if (!widget.instanceOf(et2_core_interfaces_1.et2_IDetachedDOM)) {
        ...
        if (options) {
            ...
            for (var i = 0; i < options.length && i < legacy.length; i++) {
                // Not set
                if (options[i] === "")
                    continue;
                var attr = widget.attributes[legacy[i]] || {};
        ...
        jQuery("div", row).each(function (i) {
            ...
            typeof _data[et2_historylog.columns[et2_historylog.NEW_VALUE].id] == "object" &&
                ...
                if (i === et2_historylog.NEW_VALUE) {
                    ...
                    if (nodes)
                        nodes.setDetachedAttributes(nodes, {
                            value: value,
            ...
            if (widget) {
                if (widget._children.length) {
                    ...
                    for (var j = 0; j < widget._children.length; j++) {
                        ...
                        if (widget._children[j] instanceof Et2Diff_1.Et2Diff) {
    ...
    et2_historylog.prototype._resize = function () {
        var tab = this.get_tab_info();
        if (this.dataview) {
            var style = getComputedStyle(tab.contentDiv);
            // -# to avoid scrollbars
            this.dataview.resize(Math.min(window.innerWidth - 15, parseInt(style.width)) - 5, parseInt(style.height) - parseInt(style.paddingTop) - parseInt(style.paddingBottom) - 5);
        }
    };
