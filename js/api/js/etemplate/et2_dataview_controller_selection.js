...
var et2_dataview_view_aoi_1 = require("./et2_dataview_view_aoi");
...
var et2_dataview_selectionManager = /** @class */ (function () {
    ...
    et2_dataview_selectionManager.prototype.unregisterRow = function (_uid, _tr, _noDelete) {
        ...
        if (typeof this._registeredRows[_uid] !== "undefined"
            ...
            if (!_noDelete
                && this._registeredRows[_uid].state === egw_action_constants_1.EGW_AO_STATE_NORMAL) {
    ...
    et2_dataview_selectionManager.prototype.setSelected = function (_uid, _selected) {
        this._selectAll = false;
        var entry = this._getRegisteredRowsEntry(_uid);
        this._updateEntryState(entry, egw_action_common_1.egwSetBit(entry.state, egw_action_constants_1.EGW_AO_STATE_SELECTED, _selected));
    };
    ...
    et2_dataview_selectionManager.prototype.setFocused = function (_uid, _focused) {
        ...
        if (this._focusedEntry) {
            this._updateEntryState(this._focusedEntry, egw_action_common_1.egwSetBit(this._focusedEntry.state, egw_action_constants_1.EGW_AO_STATE_FOCUSED, false));
            this._focusedEntry = null;
        }
        ...
        if (_focused) {
            ...
            this._updateEntryState(entry, egw_action_common_1.egwSetBit(entry.state, egw_action_constants_1.EGW_AO_STATE_FOCUSED, true));
        }
    };
    et2_dataview_selectionManager.prototype.selectAll = function () {
        ...
        if (egw_global_1.egw.dataKnownUIDs(this._context._dataProvider.dataStorePrefix).length !== this._total &&
    ...
    et2_dataview_selectionManager.prototype.getSelected = function () {
        ...
        var ids = [];
        for (var key in this._registeredRows) {
            if (egw_action_common_1.egwBitIsSet(this._registeredRows[key].state, egw_action_constants_1.EGW_AO_STATE_SELECTED)) {
                ids.push(key);
            }
        }
    ...
    et2_dataview_selectionManager.prototype._attachActionObjectInterface = function (_entry, _tr, _uid) {
        ...
        _entry.aoi = new et2_dataview_view_aoi_1.et2_dataview_rowAOI(_tr);
        _entry.aoi.setStateChangeCallback(function (_newState, _changedBit, _shiftState) {
            if (_changedBit === egw_action_constants_1.EGW_AO_STATE_SELECTED) {
                // Call the select handler
                this._handleSelect(_uid, _entry, egw_action_common_1.egwBitIsSet(_shiftState, egw_action_constants_1.EGW_AO_SHIFT_STATE_BLOCK), egw_action_common_1.egwBitIsSet(_shiftState, egw_action_constants_1.EGW_AO_SHIFT_STATE_MULTI));
            }
        }, this);
    };
    et2_dataview_selectionManager.prototype._getDummyAOI = function (_entry, _tr, _uid, _idx) {
        // Create AOI
        var dummyAOI = new egw_action_1.egwActionObjectInterface();
        var self = this;
        // Handling for Action Implementations updating the state
        dummyAOI.doSetState = function (_state) {
            if (!self._inUpdate) {
                // Update the "focused" flag
                self.setFocused(_uid, egw_action_common_1.egwBitIsSet(_state, egw_action_constants_1.EGW_AO_STATE_FOCUSED));
    ...
    et2_dataview_selectionManager.prototype._updateEntryState = function (_entry, _state) {
        if (this._selectAll) {
            _state |= egw_action_constants_1.EGW_AO_STATE_SELECTED;
        }
        ...
        if ((_entry.aoi && _entry.aoi.getState() !== _state) || _entry.state != _state) {
            ...
            if (_entry.ao) {
                _entry.ao.setSelected(egw_action_common_1.egwBitIsSet(_state, egw_action_constants_1.EGW_AO_STATE_SELECTED));
                _entry.ao.setFocused(egw_action_common_1.egwBitIsSet(_state, egw_action_constants_1.EGW_AO_STATE_FOCUSED));
            }
            this._inUpdate = false;
            // Delete the element if state was set to "NORMAL" and there is
            // no tr
            if (_state === egw_action_constants_1.EGW_AO_STATE_NORMAL && !_entry.tr) {
    ...
    et2_dataview_selectionManager.prototype._getRegisteredRowsEntry = function (_uid) {
        if (typeof this._registeredRows[_uid] === "undefined") {
            this._registeredRows[_uid] = {
                "uid": _uid,
                "idx": null,
                "state": egw_action_constants_1.EGW_AO_STATE_NORMAL,
                "tr": null,
                "aoi": null,
                "ao": null
            };
    ...
    et2_dataview_selectionManager.prototype._handleSelect = function (_uid, _entry, _shift, _ctrl) {
        ...
        var entry = this._getRegisteredRowsEntry(_uid);
        this.setSelected(_uid, !_ctrl || !egw_action_common_1.egwBitIsSet(entry.state, egw_action_constants_1.EGW_AO_STATE_SELECTED));
    ...
    et2_dataview_selectionManager.prototype._selectRange = function (_start, _stop) {
        ...
        for (var i = s; i <= e; i++) {
            if (typeof this._indexMap[i] !== "undefined" &&
                this._indexMap[i].uid && egw_global_1.egw.dataGetUIDdata(this._indexMap[i].uid)) {
                // Add the range to the "queryRanges"
                if (naStart !== false) {
                    queryRanges.push(et2_core_common_1.et2_bounds(naStart, i - 1));
                    naStart = false;
                    range_break += RANGE_MAX;
                }
                // Select the element, unless flagged for exclusion
                // Check for no_actions flag via data
                var data = egw_global_1.egw.dataGetUIDdata(this._indexMap[i].uid);
                ...
                queryRanges.push(et2_core_common_1.et2_bounds(naStart ? naStart : s, i - 1));
                naStart = i;
                range_break += RANGE_MAX;
            }
        }
    ...
    et2_dataview_selectionManager.prototype._query_ranges = function (queryRanges) {
        ...
        var fetchList = [fetchPromise];
        // Found after dialog loads
        var progressbar;
        var dialog = new Et2Dialog_1.Et2Dialog(this._context._widget.egw());
        dialog.transformAttributes({
            callback: 
            // Abort the long task if they canceled the data load
            function () { cont = false; },
            template: egw_global_1.egw.webserverUrl + '/api/templates/default/long_task.xet',
            message: egw_global_1.egw.lang('Loading'),
            title: egw_global_1.egw.lang('please wait...'),
            buttons: [{
            ...
            dialog.eTemplate.DOMContainer.addEventListener('load', function () {
                ...
                for (var i = 0; i < queryRanges.length; i++) {
                    if (record_count + (queryRanges[i].bottom - queryRanges[i].top + 1) > et2_dataview_selectionManager.MAX_SELECTION) {
                        egw_global_1.egw.message(egw_global_1.egw.lang('Too many rows selected.<br />Select all, or less than %1 rows', et2_dataview_selectionManager.MAX_SELECTION));
                        break;
                    }
                    else {
                            ...
                            return new Promise(function (resolve) {
                                that._queryRangeCallback.call(that._context, this, function (_order) {
                                    for (var j = 0; j < _order.length; j++) {
                                        ...
                                        var data = egw_global_1.egw.dataGetUIDdata(_order[j]);
                                        if (!data || data && data.data && !data.data.no_actions) {
                                            var entry = this._getRegisteredRowsEntry(_order[j]);
                                            this._updateEntryState(entry, egw_action_common_1.egwSetBit(entry.state, egw_action_constants_1.EGW_AO_STATE_SELECTED, true));
                                        }
                                    }
