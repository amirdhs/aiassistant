...
require("../../../vendor/tinymce/tinymce/tinymce.min.js");
...
var et2_htmlarea = /** @class */ (function (_super) {
    ...
    et2_htmlarea.prototype.init_editor = function () {
        ...
        if (this.options.imageUpload && this.options.imageUpload[0] !== '/' && this.options.imageUpload.substr(0, 4) != 'http') {
            imageUpload = egw_global_1.egw.ajaxUrl("EGroupware\\Api\\Etemplate\\Widget\\Vfs::ajax_htmlarea_upload") +
                '&request_id=' + this.getInstanceManager().etemplate_exec_id + '&widget_id=' + this.options.imageUpload + '&type=htmlarea';
            imageUpload = imageUpload.substr(egw_global_1.egw.webserverUrl.length + 1);
        }
        else if (imageUpload) {
            imageUpload = this.options.imageUpload.substr(egw_global_1.egw.webserverUrl.length + 1);
        }
        else {
            imageUpload = egw_global_1.egw.ajaxUrl("EGroupware\\Api\\Etemplate\\Widget\\Vfs::ajax_htmlarea_upload") +
        ...
        var settings = {
            base_url: egw_global_1.egw.webserverUrl + '/vendor/tinymce/tinymce',
            ...
            formats: {
                ...
                p: { block: 'p', styles: {
                        "font-family": (egw_global_1.egw.preference('rte_font', 'common') || 'arial, helvetica, sans-serif'),
                        "font-size": (egw_global_1.egw.preference('rte_font_size', 'common') || '10') +
                            (egw_global_1.egw.preference('rte_font_unit', 'common') || 'pt')
                    } },
                customparagraph: { block: 'p', styles: { "margin-block-start": "0px", "margin-block-end": "0px" } }
            },
            min_height: 100,
            convert_urls: false,
            language: et2_htmlarea.LANGUAGE_CODE[egw_global_1.egw.preference('lang', 'common')],
            language_url: egw_global_1.egw.webserverUrl + '/api/js/tinymce/langs/' + et2_htmlarea.LANGUAGE_CODE[egw_global_1.egw.preference('lang', 'common')] + '.js',
            ...
            content_css: egw_global_1.egw.webserverUrl + '/api/tinymce.php?' + // use the 3 prefs as cache-buster
                btoa(egw_global_1.egw.preference('rte_font', 'common') + '::' +
                    egw_global_1.egw.preference('rte_font_size', 'common') + '::' +
                    egw_global_1.egw.preference('rte_font_unit', 'common')),
        };
        var rte_formatblock = (egw_global_1.egw.preference('rte_formatblock', 'common') || 'p');
    ...
    et2_htmlarea.prototype._file_picker_callback = function (_callback, _value, _meta) {
        ...
        var et2;
        if (etemplate && etemplate.name && !app[egw_global_1.egw(window).app_name()]) {
            et2 = etemplate2_1.etemplate2.getByTemplate(etemplate.name)[0]['widgetContainer'];
        }
        else {
            et2 = app[egw_global_1.egw(window).app_name()].et2;
        }
        var dialog = Et2Widget_1.loadWebComponent("et2-vfs-select-dialog", {
            id: 'upload',
            mode: 'open',
            multiple: false,
            buttonLabel: "Link",
        ...
        dialog.addEventListener('change', function () {
            var file = dialog.fileInfo(Array.isArray(dialog.value) ? dialog.value[0] : dialog.value);
            callback(egw_global_1.egw.webserverUrl + file.downloadUrl, { alt: file.name });
        });
    ...
    et2_htmlarea.prototype._extendedSettings = function () {
        var rte_menubar = egw_global_1.egw.preference('rte_menubar', 'common');
        var rte_toolbar = egw_global_1.egw.preference('rte_toolbar', 'common');
        ...
        var settings = {
            fontsize_formats: et2_htmlarea.FONT_SIZE_FORMATS[egw_global_1.egw.preference('rte_font_unit', 'common')],
    ...
    et2_htmlarea.prototype.destroy = function () {
        if (this.editor) {
            try {
                ...
                egw_global_1.egw().debug("Error destroying editor", e);
            }
        }
        this.editor = null;
        this.tinymce = null;
    ...
    et2_htmlarea.prototype.set_value = function (_value) {
        ...
        if (this.editor) {
            ...
            if (this.options.applyDefaultFont) {
                window.setTimeout(function () { return _this.wrapTextNodes(); }, 10);
            }
        }
        else {
            if (this.options.readonly) {
     ...
     */
    et2_htmlarea.prototype.getValue = function (submit_value) {
        if (this.editor) {
     ...
     */
    et2_htmlarea.prototype.wrapTextNodes = function () {
        var _a;
        var body = (_a = this.editor) === null || _a === void 0 ? void 0 : _a.editorContainer.querySelector('iframe').contentDocument.querySelector('body');
        if (!body)
            return false;
        var toWrap = [];
        body.childNodes.forEach(function (node) {
            var _a;
            var text_non_block_node = node.nodeType === node.TEXT_NODE ||
                node.nodeType === node.ELEMENT_NODE && typeof node.computedStyleMap !== 'undefined' && ((_a = node.computedStyleMap().get('display')) === null || _a === void 0 ? void 0 : _a.value) !== 'block';
            if (text_non_block_node) {
                toWrap.push(node);
            }
            if ((!text_non_block_node || node === body.lastChild) && toWrap.length) {
                var wrap_1 = body.ownerDocument.createElement('p');
                toWrap.forEach(function (node) {
                    wrap_1.appendChild(node === toWrap[0] ?
                        body.replaceChild(wrap_1, node) :
    ...
    et2_htmlarea.prototype.applyDefaultFont = function () {
        var _a;
        var edit_area = (_a = this.editor) === null || _a === void 0 ? void 0 : _a.editorContainer.querySelector('iframe').contentDocument;
        if (!edit_area)
            return false;
        // we first need to wrap all text and non-block elements in p, to be able to set a font
        this.wrapTextNodes();
        var font_family = egw_global_1.egw.preference('rte_font', 'common') || 'arial, helvetica, sans-serif';
        ...
        var font_size = (egw_global_1.egw.preference('rte_font_size', 'common') || '10') + (egw_global_1.egw.preference('rte_font_unit', 'common') || 'pt');
