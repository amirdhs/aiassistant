...
var et2_placeholder_select = /** @class */ (function (_super) {
    ...
    et2_placeholder_select.prototype._buildDialog = function (_data) {
        ...
        for (var i = 0; i < non_apps.length; i++) {
            var index = data.modifications.entry.application_list.indexOf(non_apps[i]);
        ...
        this.dialog = new Et2Dialog_1.Et2Dialog(this.egw());
    ...
    et2_placeholder_select.prototype._on_template_load = function () {
        var _this = this;
        var app = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("app");
        var group = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("group");
        var placeholder_list = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("placeholder_list");
        var preview = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("preview_placeholder");
        var entry = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("entry");
        ...
        app.onchange = function (node, widget) {
            ...
            if (['user', 'filemanager'].indexOf(widget.get_value()) >= 0) {
                ...
                _this.egw().langRequireApp(_this.egw().window, widget.get_value());
                entry.set_disabled(false);
        ...
        group.onchange = function (select_node, select_widget) {
            ...
            preview.set_value("");
            placeholder_list.updateComplete.then(function () { return placeholder_list.set_value(options[0].value); });
        };
        placeholder_list.onchange = this._on_placeholder_select.bind(this);
        entry.onchange = this._on_placeholder_select.bind(this);
        this.dialog.eTemplate.widgetContainer.getDOMWidgetById("insert_placeholder").onclick = function () {
            _this.options.insert_callback(_this.dialog.eTemplate.widgetContainer.getDOMWidgetById("preview_placeholder").getDOMNode().textContent);
        };
        this.dialog.eTemplate.widgetContainer.getDOMWidgetById("insert_content").onclick = function () {
            _this.options.insert_callback(_this.dialog.eTemplate.widgetContainer.getDOMWidgetById("preview_content").getDOMNode().textContent);
        };
    ...
    et2_placeholder_select.prototype._on_placeholder_select = function () {
        ...
        var placeholder_list = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("placeholder_list");
        var preview = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("preview_placeholder");
        var preview_content = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("preview_content");
        // Show the selected placeholder
        this.set_value(placeholder_list.get_value());
        preview.set_value(placeholder_list.get_value());
        preview.getDOMNode().parentNode.style.visibility = ((_a = placeholder_list.get_value()) === null || _a === void 0 ? void 0 : _a.trim()) ? null : 'hidden';
...
var et2_placeholder_snippet_select = /** @class */ (function (_super) {
     ...
     * @memberOf et2_vfsSelect
     */
    function et2_placeholder_snippet_select(_parent, _attrs, _child) {
        var _this = 
        // Call the inherited constructor
        _super.call(this, _parent, _attrs, et2_core_inheritance_1.ClassWithAttributes.extendAttributes(et2_placeholder_select._attributes, _child || {})) || this;
        _this.LIST_URL = 'EGroupware\\Api\\Etemplate\\Widget\\Placeholder::ajax_get_placeholders';
        ...
        // Load app translations
        _this.egw().langRequireApp(_this.egw().window, "addressbook");
        return _this;
    }
    /**
     * Post-load of the dialog
    ...
    et2_placeholder_snippet_select.prototype._on_template_load = function () {
        ...
        var app = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("app");
        var placeholder_list = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("placeholder_list");
        var preview = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("preview_content");
        var entry = this.dialog.eTemplate.widgetContainer.getDOMWidgetById("entry");
        ...
        app.onchange = function (node, widget) {
            ...
            placeholder_list.set_select_options(_this._get_placeholders(app.value, "addresses"));
        };
        placeholder_list.onchange = this._on_placeholder_select.bind(this);
        entry.onchange = this._on_placeholder_select.bind(this);
    ...
    et2_placeholder_snippet_select.prototype._on_placeholder_select = function () {
        ...
        var placeholder = "";
        if (app && app.value) {
            placeholder = Object.keys(et2_placeholder_snippet_select.placeholders[app.value]["addresses"])[placeholder_list.value];
        }
        if (placeholder && entry.get_value()) {
            // Show the selected placeholder replaced with value from the selected entry
            this.egw().json('EGroupware\\Api\\Etemplate\\Widget\\Placeholder::ajax_fill_placeholders', [placeholder, { app: "addressbook", id: entry.get_value() }], function (_content) {
                if (!_content) {
                    _content = '';
                }
    ...
    et2_placeholder_snippet_select.prototype._get_placeholders = function (appname, group) {
        var _this = this;
        var options = [];
        Object.keys(et2_placeholder_snippet_select.placeholders[appname][group]).map(function (key, index) {
            options.push({
                value: index,
